...
export default function ProjectDetailFaq({ project }: { project: Project }) {
  const faqs = [
    {
      question: `Where is ${project.title} located?`,
      answer: `The project is in ${project.location}. It was built by BRU CO. (Building Reference United), a general contractor headquartered in Jeddah and working across Jeddah, Makkah, the Western Region and the wider Kingdom of Saudi Arabia.`,
    },
    {
      question: `What was the scope of work on ${project.title}?`,
      answer: `The scope covered ${project.scope.toLowerCase()}. BRU CO. delivers as a single general contractor, so structural, architectural, electromechanical (MEP) and interior finishing works sit with one accountable party rather than being split across separate trades contracts.`,
    },
    {
      question: `How large is ${project.title}?`,
      answer: `${project.size ?? "This project"}. The project sits in the ${project.category.toLowerCase()} sector, one of more than 300 projects BRU CO. has completed in Saudi Arabia.`,
    },
    {
      question: `Which construction company built ${project.title}?`,
      ...
      <div className="mx-auto grid max-w-full gap-12 px-6 sm:px-8 lg:grid-cols-[0.8fr_1.2fr] lg:gap-20 lg:px-12">
        <div className="flex flex-col gap-6 lg:sticky lg:top-32 lg:h-fit">
          ...
          <Reveal tag="p" delay={140} className="max-w-md text-[1.0625rem] leading-[1.55] tracking-[-0.011em] text-dust">
            What clients usually ask before building something similar with a
            general contractor in Jeddah.
          </Reveal>
        </div>
