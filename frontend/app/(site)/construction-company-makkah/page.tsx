...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const data = await getLocationPage("makkah", locale);
  return { title: data.seo.title, description: data.seo.description };
}

export default async function ConstructionCompanyMakkahPage() {
  const locale = await getLocale();
  const [data, projects] = await Promise.all([getLocationPage("makkah", locale), spotlightProjects()]);
