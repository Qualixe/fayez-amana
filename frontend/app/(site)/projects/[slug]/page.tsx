...
import { categoryLabel, localize } from "@/components/projects/data";
import { getProject, adjacentProjects, getProjectDetailSettings, getMethodArticles } from "@/lib/db/projects";
...
export default async function ProjectDetailPage(props: PageProps<"/projects/[slug]">) {
  ...
  const locale = await getLocale();
  const [{ prev, next }, detailSettings, methodArticles] = await Promise.all([
    adjacentProjects(slug),
    getProjectDetailSettings(locale),
    getMethodArticles(locale),
  ]);
  ...
  return (
    <>
      ...
      <ProjectDetailMethod settings={detailSettings.method} articles={methodArticles} />
