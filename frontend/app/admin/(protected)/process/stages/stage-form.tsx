const fieldClasses = "w-full border border-steel bg-void px-3 py-2.5 text-bone outline-none focus:border-azure-lift";
const labelClasses = "flex flex-col gap-1.5 text-sm text-dust";

export type StageFormValues = { no?: number; title?: string; title_ar?: string; body?: string; body_ar?: string };

export default function StageForm({
  action,
  defaultValues,
  submitLabel,
  lockNo,
}: {
  action: (formData: FormData) => void;
  defaultValues?: StageFormValues;
  submitLabel: string;
  lockNo?: boolean;
}) {
  const v = defaultValues ?? {};
  return (
    <form action={action} className="flex max-w-2xl flex-col gap-4">
      <label className={labelClasses}>
        Stage number
        <input
          name="no"
          type="number"
          required
          readOnly={lockNo}
          defaultValue={v.no}
          className={`${fieldClasses} ${lockNo ? "opacity-60" : ""}`}
        />
      </label>
      <label className={labelClasses}>
        Title (English)
        <input name="title" required defaultValue={v.title} className={fieldClasses} />
      </label>
      <label className={labelClasses}>
        العنوان (عربي)
        <input name="title_ar" dir="rtl" required defaultValue={v.title_ar} className={fieldClasses} />
      </label>
      <label className={labelClasses}>
        Body (English)
        <textarea name="body" required rows={5} defaultValue={v.body} className={fieldClasses} />
      </label>
      <label className={labelClasses}>
        الوصف (عربي)
        <textarea name="body_ar" dir="rtl" required rows={5} defaultValue={v.body_ar} className={fieldClasses} />
      </label>
      <button
        type="submit"
        className="w-fit bg-azure px-6 py-3 font-mono text-[11px] uppercase tracking-[0.18em] text-white transition-[filter] hover:brightness-110"
      >
        {submitLabel}
      </button>
    </form>
  );
}
