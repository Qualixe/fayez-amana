...
export default async function AdminCategoriesPage() {
  ...
  const { data: rows, error } = await supabase
    ...
    .select("id, key, label, from_stage, to_stage")
  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-semibold tracking-[-0.02em] text-bone">Stage categories</h1>
        <Link
          href="/admin/process/categories/new"
      ...
      <table className="w-full border-collapse text-start text-sm">
        <thead>
          <tr className="border-b border-steel font-mono text-[10px] uppercase tracking-[0.18em] text-dust">
            ...
            <th className="py-2 pe-4 text-start font-medium">Range</th>
            ...
            <tr key={row.id} className="border-b border-steel/60">
              ...
              <td className="py-3 text-end">
                <div className="flex justify-end gap-4">
                  <Link href={`/admin/process/categories/${row.id}`} className="text-azure-glow hover:underline">
                    Edit
                  </Link>
                  <form action={async () => { "use server"; await deleteCategory(row.id); }}>
