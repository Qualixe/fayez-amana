...
import { deleteVisionItem } from "./actions";

export default async function AdminVisionItemsPage() {
  const supabase = await createClient();
  const { data: rows, error } = await supabase
    .from("about_vision_items")
  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-semibold tracking-[-0.02em] text-bone">Vision & mission items</h1>
        <Link
          href="/admin/about/vision-items/new"
            ...
            <tr key={row.id} className="border-b border-steel/60">
              ...
              <td className="py-3 text-end">
                <div className="flex justify-end gap-4">
                  <Link href={`/admin/about/vision-items/${row.id}`} className="text-azure-glow hover:underline">
                    Edit
                  </Link>
                  <form action={async () => { "use server"; await deleteVisionItem(row.id); }}>
