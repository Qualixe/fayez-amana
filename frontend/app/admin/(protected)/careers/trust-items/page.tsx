    ...
    <div className="flex flex-col gap-6">
      <div className="flex items-center justify-between">
        <div className="flex flex-col gap-1">
          <h1 className="text-2xl font-semibold tracking-[-0.02em] text-bone">Careers — Application trust badges</h1>
          <p className="text-sm text-dust">The small trust cards (Confidential, Response time, etc.) shown below the application form.</p>
        </div>
        <Link
          href="/admin/careers/trust-items/new"
            ...
            <tr key={row.id} className="border-b border-steel/60">
              ...
              <td className="py-3 text-end">
                <div className="flex justify-end gap-4">
                  <Link href={`/admin/careers/trust-items/${row.id}`} className="text-azure-glow hover:underline">
                    Edit
                  </Link>
                  <form action={async () => { "use server"; await deleteTrustItem(row.id); }}>
      ...
      {!rows?.length ? <p className="text-sm text-dust">No trust badges yet — add one.</p> : null}
    </div>
  );
}
