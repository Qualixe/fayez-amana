...
export default async function AdminCultureItemsPage() {
  const supabase = await createClient();
  const { data: rows, error } = await supabase
    .from("career_culture_items")
    .select("id, title")
    ...
    <div className="flex flex-col gap-6">
      <div className="flex items-center justify-between">
        <div className="flex flex-col gap-1">
          <h1 className="text-2xl font-semibold tracking-[-0.02em] text-bone">Careers — Company culture</h1>
          <p className="text-sm text-dust">The 4 cards shown under &quot;Company Culture&quot; on the Careers page.</p>
        </div>
        <Link
          href="/admin/careers/culture/new"
            ...
            <tr key={row.id} className="border-b border-steel/60">
              ...
              <td className="py-3 text-end">
                <div className="flex justify-end gap-4">
                  <Link href={`/admin/careers/culture/${row.id}`} className="text-azure-glow hover:underline">
                    Edit
                  </Link>
                  <form action={async () => { "use server"; await deleteCultureItem(row.id); }}>
