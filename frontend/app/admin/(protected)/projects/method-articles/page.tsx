...
import { deleteMethodArticle } from "./actions";

export default async function AdminMethodArticlesPage() {
            ...
            <tr key={row.id} className="border-b border-steel/60">
              ...
              <td className="py-3 text-end">
                <div className="flex justify-end gap-4">
                  <Link href={`/admin/projects/method-articles/${row.id}`} className="text-azure-glow hover:underline">
                    Edit
                  </Link>
                  <form action={async () => { "use server"; await deleteMethodArticle(row.id); }}>
