    ...
    <div className="flex flex-col gap-6">
      <div className="flex items-center justify-between">
        <div className="flex flex-col gap-1">
          <h1 className="text-2xl font-semibold tracking-[-0.02em] text-bone">Core values</h1>
          <p className="text-sm text-dust">
            Shared by the Home page and the About page. See also{" "}
            <Link href="/admin/home/values-settings" className="text-azure-glow hover:underline">
              section settings
            </Link>
            .
          </p>
      ...
      <table className="w-full border-collapse text-start text-sm">
        <thead>
          <tr className="border-b border-steel font-mono text-[10px] uppercase tracking-[0.18em] text-dust">
            <th className="py-2 pe-4 text-start font-medium">Letter</th>
        ...
        <tbody>
          {(rows ?? []).map((row) => (
            <tr key={row.id} className="border-b border-steel/60">
              <td className="py-3 pe-4 font-mono text-dust">{row.letter}</td>
              <td className="py-3 pe-4 text-bone">{row.title}</td>
              <td className="py-3 text-end">
                <div className="flex justify-end gap-4">
                  <Link href={`/admin/home/values/${row.id}`} className="text-azure-glow hover:underline">
                    Edit
                  </Link>
                  <form action={async () => { "use server"; await deleteValue(row.id); }}>
